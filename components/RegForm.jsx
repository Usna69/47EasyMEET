...
export default function RegForm({ meetingprop }) {
  ...
  const meetingId = meetingprop.id;
  ...
  useEffect(() => {
    const fetchMeetingData = async () => {
      ...
      try {
        ...
        console.log(
          "Status calculation - Meeting time:",
          meetingTime.toLocaleString()
        );
        console.log(
          "Status calculation - Registration end time:",
          registrationEndTime.toLocaleString()
        );
        ...
        console.log(
          "Status calculation - Is meeting ended (more than a day old):",
          isMeetingEnded
        );
        ...
        if (now >= meetingTime) {
          ...
          if (now <= registrationEndTime && !isMeetingEnded) {
            status = "ONGOING";
            console.log(
              "Status calculation - Meeting is ONGOING: Within registration window"
            );
          } else {
            status = "CLOSED";
            console.log(
              "Status calculation - Meeting is CLOSED: Registration period ended"
            );
          }
        } else {
          console.log(
            "Status calculation - Meeting is UPCOMING: Meeting has not started yet"
          );
        }
      ...
      {meeting && (
        ...
        >
          ...
          <p className="text-xs mt-1">
            {meeting.status === "ONGOING"
              ? `Registration closes at ${new Date(
                  meeting.registrationEndTime
                ).toLocaleTimeString()}`
              : meeting.status === "UPCOMING"
              ? `Registration opens when the meeting starts at ${new Date(
                  meeting.date
                ).toLocaleTimeString()}`
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mt-4">
        <div>
          ...
          <input
            ...
            } rounded-md shadow-sm focus:outline-none focus:ring-[#014a2f]/20 focus:border-[#014a2f]`}
          ...
          {loading ? (
            ...
            <div>
              ...
              <button
                type="button"
                disabled={
                  isSubmitting || !meeting || meeting.status !== "ONGOING"
                }
                className={`w-full py-3 px-4 rounded-md font-medium transition-colors ${
                  meeting && meeting.status === "ONGOING"
                    ? "bg-yellow-400 hover:bg-yellow-500 text-[#014a2f]"
